...
import type { PrizeMachineConfig, PrizeSpinPrize } from "./prize-machine";
...
export function playPrizeMachineSpin(
  ...
  const centerY = gameHeight * 0.535;
...
function createPrizeSpinnerShell(
  ...
  ring = createPrizeRing(scene, segments, gameWidth / 2, gameHeight * 0.535, 152)
): Phaser.GameObjects.Container {
  const centerX = gameWidth / 2;
  return scene.add.container(0, 0, [
    scene.add.rectangle(gameWidth / 2, gameHeight / 2, gameWidth, gameHeight, 0x011827, 0.86),
    createTitle(scene, centerX, 92, config.title, "34px", "#ffffff"),
    ...headerChildren,
    ring.container,
    ...children
  ]).setDepth(95);
...
function createBalanceText(scene: Phaser.Scene, x: number, commonCoins: number): Phaser.GameObjects.Text {
  return createTitle(scene, x, 132, commonCoinLabel(commonCoins), "21px", "#ffe67a");
}
...
function createPrizeRing(
  ...
  for (let index = 0; index < segmentCount; index += 1) {
    ...
    const cell = createPrizeCell(scene, segment, x, y, index === 0);
    cells.push(cell);
...
function createPrizeCell(scene: Phaser.Scene, segment: PrizeWheelSegment, x: number, y: number, isJackpot: boolean): PrizeRingCell {
  const radius = 43;
  ...
  const highlight = scene.add.circle(0, 0, radius, 0xffffff, 0.08)
    ...
    .setAlpha(0);
  const icon = scene.add.image(0, -6, segment.iconTextureKey);
  icon.setDisplaySize(40, 40);
  const label = createSegmentLabel(scene, 0, 22, segment.label);
...
function createSegmentLabel(scene: Phaser.Scene, x: number, y: number, label: string): Phaser.GameObjects.Text {
  return scene.add.text(x, y, label, {
    fontFamily: gameFontFamily,
    fontSize: "10px",
    color: "#ffffff",
